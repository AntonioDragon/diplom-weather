...
import {WeatherElement} from '../../../../app/weather/fiveDaysThreeHour'
...
const SliderCardHour: React.FC<SliderCardWeatherProps> = ({weather}) => {
  return (
    <SliderCard>
      <BlockFlex justifyContent={JustifyContentEnum.spaceBetween}>
        ...
        <div>
          <SliderHeader>
            {weather && format(new Date(weather?.dt_txt!), 'h aaa')}
          </SliderHeader>
          <SliderParagraph>{Math.ceil(weather?.main.temp)}</SliderParagraph>
          <BlockFlex>
            <SliderIcon
      ...
      <BlockFlex justifyContent={JustifyContentEnum.spaceBetween}>
        <div>
          <BlockFlex>
            ...
            <SliderParagraph>{Math.ceil(weather?.clouds.all)}%</SliderParagraph>
          </BlockFlex>
          <BlockFlex>
            <SliderIcon
              xmlns='http://www.w3.org/2000/SliderIcon'
            ...
            <SliderParagraph>
              {weather?.rain ? Math.ceil(weather?.rain['3h'] * 100) : 0}%
            </SliderParagraph>
          </BlockFlex>
        </div>
        <div>
          <BlockFlex>
            ...
            <SliderParagraph>{weather?.wind.speed}m/s</SliderParagraph>
          </BlockFlex>
          <BlockFlex>
            <SliderIcon
              xmlns='http://www.w3.org/2000/SliderIcon'
            ...
            <SliderParagraph>{weather?.main.humidity}%</SliderParagraph>
          </BlockFlex>
        </div>
      </BlockFlex>
      <SliderParagraph>
        <span>Pressure:</span> {weather?.main.pressure} Pa
      </SliderParagraph>
    </SliderCard>
  )
}

export default SliderCardHour
