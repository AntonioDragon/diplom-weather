...
import {HourlyTwoDaysElement} from '../../../../app/weather/hourlyTwoDays'
...
interface SliderCardWeatherProps {
  weather: HourlyTwoDaysElement
}

const SliderCardHourlyTwoDays: React.FC<SliderCardWeatherProps> = ({
  weather
}) => {
  return (
    <SliderCard>
      <BlockFlex justifyContent={JustifyContentEnum.spaceBetween}>
        ...
        <div>
          <SliderHeader>
            {weather && format(new Date(weather?.dt! * 1000), 'h aaa')}
          </SliderHeader>
          <SliderParagraph>{Math.ceil(weather?.temp)}</SliderParagraph>
          <BlockFlex>
            <SliderIcon
      ...
      <BlockFlex justifyContent={JustifyContentEnum.spaceBetween}>
        <div>
          ...
          <BlockFlex>
            ...
            <SliderParagraph>
              {Math.ceil(weather?.rain ? weather?.rain['1h'] * 100 : 0)}%
            </SliderParagraph>
          </BlockFlex>
        </div>
        <div>
          <BlockFlex>
            ...
            <SliderParagraph>
              {Math.ceil(weather?.wind_speed)}m/s
            </SliderParagraph>
          </BlockFlex>
          <BlockFlex>
            <SliderIcon
...
export default SliderCardHourlyTwoDays
